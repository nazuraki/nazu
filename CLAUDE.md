...
- **usr (users/permissions):** `apps/usr` owns users, app-scoped roles and
  ...
  emails). Hot path for other apps: `GET /api/permissions?email=&app=` with
  `USR_API_KEY`. Auth ladder mirrors the web app (API key → session → Basic →
  zero-conf open); OAuth config is DB-backed. See [ADR 0004](docs/adr/0004-usr-user-management.md).
